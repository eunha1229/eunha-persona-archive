...
function showDetail(i){const p=state.posts[i];if(!p)return;navActive("");setCrumb(p.codename||p.name);const sections=p.sections.map((s,si)=>{const prompt=s.type==="copy"||/PROMPT|OOC/i.test(s.title);return`<section class="doc-section"><h2>${esc(s.title)}</h2>${prompt?`<div class="prompt-box"><div class="prompt-top"><span>COPY SECTION</span><button class="copy-btn" onclick="copyPrompt(${i},${si},this)">COPY</button></div><pre>${esc(s.content)}</pre></div>`:`<div class="prose">${esc(s.content)}</div>`}</section>`}).join("");contentEl.innerHTML=`<button class="back-btn" onclick="showPersonas()">← BACK TO INDEX</button><div class="detail-tools"><button class="delete-entry-btn" onclick="deletePersona(${i})">DELETE ENTRY</button><button class="edit-entry-btn" onclick="editor(${i})">✎ EDIT ENTRY</button></div><div class="detail-head">${art(p,"portrait")}<div class="detail-title"><div class="eyebrow">${esc(p.world||"UNFILED")}</div><h1>${esc(p.name)}</h1>${p.native_name?`<div class="native-name">${esc(p.native_name)}</div>`:""}<div class="code">${esc(p.codename)}</div><div class="detail-tags">${p.tags.map(t=>`<span class="tag-chip">#${esc(t)}</span>`).join("")}</div></div></div>${profileGrid(p)}<div class="detail-body">${sections}</div>`;window.scrollTo(0,0);closeSide()}
...
async function ghPut(path,contentB64,message){const c=requireGitHub(),old=await ghGet(path),body={message,content:contentB64,branch:c.branch||"main"};if(old?.sha)body.sha=old.sha;return ghRequest(path,{method:"PUT",body:JSON.stringify(body)})}
...
$("ghForget").onclick=()=>{sessionStorage.removeItem("personaArchiveToken");$("ghToken").value="";updateConnectUI()};
loadAll();