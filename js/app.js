...
function buildMd(imagePath="",entrySlug=""){const d=formData(),old=editingIndex!==null?state.posts[editingIndex]:null,img=imagePath||old?.image||"",slug=entrySlug||old?.slug||postSlug(old)||slugify([d.world,d.codename||d.name].filter(Boolean).join("-"));return`---\nname: ${d.name}\nnative_name: ${d.native_name}\ncodename: ${d.codename}\nworld: ${d.world}\nslug: ${slug}\ntags: [${d.tags.join(", ")}]\nimage: ${img}\nupdated: ${new Date().toISOString().slice(0,10)}\nage: ${d.age}\ngender: ${d.gender}\nbirthday: ${d.birthday}\nheight: ${d.height}\nnationality: ${d.nationality}\npartner: ${d.partner}\n---\n\n${d.sections.map(s=>`${s.type==="copy"?"<!-- persona-archive:copy -->\n":""}# ${s.title}\n${s.content}`).join("\n\n")}\n`}
...
async function savePersonaToGitHub(){const status=$("saveStatus");try{status.textContent="게시글과 이미지를 GitHub에 저장 중…";const d=formData(),old=editingIndex!==null?state.posts[editingIndex]:null;const entrySlug=old?(old.slug||postSlug(old)):await uniquePersonaSlug(d);let filename=old?(old._file||entrySlug+".md"):(entrySlug+".md"),imagePath=old?.image||"";
 ...
 const mdText=buildMd(imagePath,entrySlug);await ghPut(`posts/${filename}`,utf8b64(mdText),`${old?"Update":"Add"} persona: ${d.name}`);
